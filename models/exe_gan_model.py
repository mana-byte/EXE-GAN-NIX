...
class EXE_GAN:
    ...
    def __init__(
        ...
        self.generator = Generator(
            ...
            psp_start_latent=psp_start_latent,
            num_psp_latent=num_psp_latent,
        ).to(device)
        ...
        self.psp_embedding = Psp_Embedding(
            psp_ckpt_path, psp_start_latent, num_psp_latent
        ).to(device)

        print("load models:", exe_ckpt_path)
        ckpt = torch.load(
            exe_ckpt_path, map_location=lambda storage, loc: storage, weights_only=False
        )
    ...
    def mixing_forward(self, real_img, mask_01, infer_imgs_a, infer_imgs_b):
        ...
        noise = mixing_noise(
            gin.size(0), self.latent, prob=self.mixing, device=self.device
        )
        ...
        with torch.no_grad():
            ...
            for jj in range(infer_embeddings_a.shape[1] + 1):
                # if jj == 3: break
                mixed_latent = self.latent_mixing(
                    infer_embeddings_a, infer_embeddings_b, jj
                )

                fake_img = self.generator(gin, mixed_latent, noise)
                completed_img = get_completion(
                    fake_img, real_img.detach(), mask_01.detach()
                )
